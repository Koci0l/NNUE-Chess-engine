...
int alphaBeta(chess::Board& board, int depth, int alpha, int beta, int ply_from_root,
    ...
    // [C] Policy pseudo-TT on IIR (TT-miss) nodes.
    //     tt_is_policy marks an injected move: it gets ordering + first-move
    //     privilege only, never real-hash-move semantics (SE / small-probcut).
    bool tt_is_policy = false;
    if (!in_singular_search && !is_pv_node && depth >= 4 && tt_move == chess::Move() && !in_check) {
        if (g_policy.loaded && depth >= POLICY_IIR_MIN_DEPTH && ply_from_root <= POLICY_IIR_MAX_PLY) {
            chess::Move pol_mv;
            float margin = 0.0f;
            if (g_policy.topMoveMargin(board, pol_mv, margin) &&
                pol_mv != chess::Move() && margin >= POLICY_IIR_MIN_MARGIN) {
                tt_move = pol_mv;
                tt_is_policy = true;
            } else {
                depth--;
    ...
    // [C] require a real hash move: a policy-injected tt_move must not enable this.
    if (!in_singular_search && !is_pv_node && tt_move != chess::Move() && !tt_is_policy &&
    ...
    while (true) {
        ...
        int se_ext = 0;
        // [C] never run singular extension on a policy-injected move.
        if (!in_singular_search && move == tt_move && !in_check && !tt_is_policy) {
