...
// [PATH B] In-tree policy: compute the policy at nodes with remaining depth
...
constexpr float POLICY_BLEND_MIN_CONF  = 0.30f;   // min policy confidence to consider
...
int alphaBeta(chess::Board& board, int depth, int alpha, int beta, int ply_from_root,
    ...
    // ========================================================================
    // [PATH B] Compute in-tree policy at high-depth nodes.
    // Uses g_policy (4096).  To use a cheaper smaller net here, load a second
    // PolicyNet (e.g. g_policy_tree, HL=512) and call computeRootPolicy with it.
    // ========================================================================
    RootPolicy nodePolicy;
    const bool haveTreePolicy =
        depth >= POLICY_TREE_MIN_DEPTH &&
        !in_check &&
        !in_singular_search &&
        computeRootPolicy(board, nodePolicy);
    ...
    MovePicker mp(board, mpCtx, depth, false, /*use_policy=*/haveTreePolicy);
    mp.setPolicy(haveTreePolicy ? &nodePolicy : nullptr);   // [PATH B]
    ...
    while (true) {
        ...
        if (can_reduce) {
            ...
            // [PATH B] In-tree policy LMR adjustment (mirrors root LMR).
            if (haveTreePolicy && nodePolicy.ok) {
                int idx = nodePolicy.find(move);
                if (idx >= 0 && nodePolicy.quiet_rank[idx] >= 0) {
                    float adj = -0.85f * nodePolicy.rel[idx] * nodePolicy.quiet_sharpness;
...
chess::Move search(chess::Board& board, int max_depth, ThreadInfo& thread, TimeManager& tm,
    ...
    // [ROOT BLEND] Track the policy-top move's search score for the tiebreak.
    int policy_top_score = -MATE_SCORE;
    ...
    for (int depth = 1; depth <= max_depth; ++depth) {
        ...
        while (true) {
            ...
            auto scoreRootMove = [&](const chess::Move& move, bool& is_quiet_out) -> int {
                ...
                if (rootPolicy.ok) {
                    ...
                    if (idx >= 0 && rootPolicy.quiet_rank[idx] >= 0) {
                        ...
                        int rank_bonus = (r == 0) ? 6000 : (r == 1) ? 3500 :
                                         (r <= 2) ? 2000 : (r <= 5) ? 900 :
                                         (r <= 10) ? 250 : 0;
            ...
            for (size_t rmi = 0; rmi < root_moves.size(); ++rmi) {
                ...
                // [ROOT BLEND] Track the policy-top move's score.
                if (rootPolicy.ok && move == rootPolicy.top_any)
                    policy_top_score = eval;
...
search_done:
    ...
    // ========================================================================
    // [ROOT BLEND] Policy-guided root tiebreak.
    // If the search's best move and the policy's top move disagree, the search
    // score is not a mate, and the policy is confident enough that its
    // log-probability outweighs the search margin, prefer the policy's move.
    // This is the "hard score + soft prior" blend: the policy only overrides
    // when the search is uncertain (small margin) and the policy is confident.
    // ========================================================================
    if (rootPolicy.ok &&
        rootPolicy.top_any != chess::Move() &&
        rootPolicy.top_any != best_move &&
        std::abs(best_score) < MATE_SCORE - 200 &&
        rootPolicy.top_prob_any >= POLICY_BLEND_MIN_CONF &&
        policy_top_score > -MATE_SCORE + 200) {

        int margin = best_score - policy_top_score;
        float threshold = POLICY_BLEND_LAMBDA *
                          (-std::log(std::max(rootPolicy.top_prob_any, 1e-6f)));
        if (float(margin) < threshold) {
            if (!g_silent) {
                std::cout << "info string policy_blend override"
                          << " search " << chess::uci::moveToUci(best_move)
                          << " (" << best_score << "cp)"
                          << " -> policy " << chess::uci::moveToUci(rootPolicy.top_any)
                          << " (" << policy_top_score << "cp,"
                          << " p=" << (rootPolicy.top_prob_any * 100.f) << "%,"
                          << " margin=" << margin << ")"
                          << std::endl;
            }
