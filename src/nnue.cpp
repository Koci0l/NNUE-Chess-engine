...
// Helper to find a king square on the board
static inline chess::Square getKingSq(const chess::Board& board, chess::Color c) {
    for (int i = 0; i < 64; ++i) {
        chess::Square sq(i);
...
// ============================================================================
usize NNUE::feature(chess::Color perspective, chess::Color color, chess::PieceType piece, chess::Square square, chess::Square wk, chess::Square bk) {
    const int colorIndex = (perspective == color) ? 0 : 1;
    
    // Vertical mirroring for Black's perspective (standard)
    int sq_idx = (perspective == chess::Color::BLACK) ? (square.index() ^ 56) : square.index();
    
    // Horizontal mirroring: If the perspective's king is on files E, F, G, H (index % 8 > 3), mirror horizontally
    chess::Square ksq = (perspective == chess::Color::WHITE) ? wk : bk;
    if ((ksq.index() % 8) > 3) {
        sq_idx ^= 7;
    }
    
    return colorIndex * 384 + static_cast<int>(piece) * 64 + sq_idx;
}
...
// Accumulator Updates
// ============================================================================
void AccumulatorPair::add_piece(const chess::Piece& p, const chess::Square& sq, chess::Square wk, chess::Square bk) {
    if (p == chess::Piece::NONE) return;
    const usize featureW = NNUE::feature(chess::Color::WHITE, p.color(), p.type(), sq, wk, bk);
    const usize featureB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), sq, wk, bk);
    for (usize i = 0; i < HL_SIZE; ++i) {
...
void AccumulatorPair::remove_piece(const chess::Piece& p, const chess::Square& sq, chess::Square wk, chess::Square bk) {
    if (p == chess::Piece::NONE) return;
    const usize featureW = NNUE::feature(chess::Color::WHITE, p.color(), p.type(), sq, wk, bk);
    const usize featureB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), sq, wk, bk);
    for (usize i = 0; i < HL_SIZE; ++i) {
...
void AccumulatorPair::move_piece(const chess::Piece& p, const chess::Square& from, const chess::Square& to, chess::Square wk, chess::Square bk) {
    if (p == chess::Piece::NONE) return;
    const usize featureFromW = NNUE::feature(chess::Color::WHITE, p.color(), p.type(), from, wk, bk);
    const usize featureFromB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), from, wk, bk);
    const usize featureToW = NNUE::feature(chess::Color::WHITE, p.color(), p.type(), to, wk, bk);
    const usize featureToB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), to, wk, bk);
    for (usize i = 0; i < HL_SIZE; ++i) {
...
void AccumulatorPair::resetAccumulators(const chess::Board& board) {
    ...
    chess::Square wk = getKingSq(board, chess::Color::WHITE);
    chess::Square bk = getKingSq(board, chess::Color::BLACK);
