...
// ============================================================================
// Input Bucket Calculation (Mirrored)
// ============================================================================
InputBucketInfo NNUE::getInputBucketInfo(chess::Square ksq) {
    int abs_sq = ksq.index();
    int file = abs_sq % 8;
    int rank = abs_sq / 8;
    
    // If king is on files E-H (4-7), we mirror the board horizontally
    int flip = (file > 3) ? 7 : 0;
    int mirrored_file = file ^ flip; // Maps 0-7 to 0-3
    
    usize bucket_idx = BUCKET_LAYOUT[rank * 4 + mirrored_file];
    return {bucket_idx, flip};
}
...
usize NNUE::feature(chess::Color perspective, chess::Color color, chess::PieceType piece, chess::Square square, usize bucket, int flip) {
    ...
    int sq_idx = square.index();
    sq_idx ^= flip; // Apply horizontal mirroring based on king's file
    
    const int squareIndex = (perspective == chess::Color::BLACK)
        ? (sq_idx ^ 56) // Vertical flip for black's perspective
        : sq_idx;
        
    return bucket * 768 + colorIndex * 384 + static_cast<int>(piece) * 64 + squareIndex;
}
...
void AccumulatorPair::add_piece(const chess::Piece& p, const chess::Square& sq) {
    ...
    const usize featureB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), sq, black.bucket, black.flip);
    for (usize i = 0; i < HL_SIZE; ++i) {
...
void AccumulatorPair::move_piece(const chess::Piece& p, const chess::Square& from, const chess::Square& to) {
    if (p == chess::Piece::NONE) return;
    const usize featureFromW = NNUE::feature(chess::Color::WHITE, p.color(), p.type(), from, white.bucket, white.flip);
    const usize featureFromB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), from, black.bucket, black.flip);
    const usize featureToW = NNUE::feature(chess::Color::WHITE, p.color(), p.type(), to, white.bucket, white.flip);
    const usize featureToB = NNUE::feature(chess::Color::BLACK, p.color(), p.type(), to, black.bucket, black.flip);
    
    for (usize i = 0; i < HL_SIZE; ++i) {
        white.values[i] += g_nnue.weightsToHL[featureToW * HL_SIZE + i] - g_nnue.weightsToHL[featureFromW * HL_SIZE + i];
        black.values[i] += g_nnue.weightsToHL[featureToB * HL_SIZE + i] - g_nnue.weightsToHL[featureFromB * HL_SIZE + i];
    }
}
...
void AccumulatorPair::resetAccumulators(const chess::Board& board) {
    ...
    auto white_info = NNUE::getInputBucketInfo(board.kingSq(chess::Color::WHITE));
    white.bucket = white_info.bucket;
    white.flip = white_info.flip;
    
    auto black_info = NNUE::getInputBucketInfo(board.kingSq(chess::Color::BLACK));
    black.bucket = black_info.bucket;
    black.flip = black_info.flip;
