...
static bool process_command(const std::string& line,
                            chess::Board& board,
                            ThreadInfo& thread) {
    ...
    } else if (command == "policyinfo") {
        std::cout << "info string policy toggles"
                  << " l2_out_major="    << (g_policy.l2_out_major ? 1 : 0)
                  << " l1_hidden_major=" << (g_policy.l1_hidden_major ? 1 : 0)
                  << " see_invert="      << (g_policy.see_invert ? 1 : 0)
                  << " feat_swap="       << (g_policy.feat_swap ? 1 : 0)
                  << " act_mode="        << g_policy.act_mode
                  << " (0=crelu01, 1=relu, 2=identity, 3=screlu01)"
                  << std::endl;

    } else if (command == "policyreset") {
        g_policy.l2_out_major    = true;
        g_policy.l1_hidden_major = true;
        g_policy.see_invert      = false;
        g_policy.feat_swap       = false;
        g_policy.act_mode        = 0;

        std::cout << "info string policy toggles reset to defaults" << std::endl;

    } else if (command == "policyflip") {
        g_policy.l2_out_major = !g_policy.l2_out_major;

        std::cout << "info string l2_out_major="
                  << (g_policy.l2_out_major ? 1 : 0)
                  << std::endl;

    } else if (command == "policyh1flip") {
        g_policy.l1_hidden_major = !g_policy.l1_hidden_major;

        std::cout << "info string l1_hidden_major="
                  << (g_policy.l1_hidden_major ? 1 : 0)
                  << std::endl;

    } else if (command == "policyseeflip") {
        g_policy.see_invert = !g_policy.see_invert;

        std::cout << "info string policy_see_invert="
                  << (g_policy.see_invert ? 1 : 0)
                  << std::endl;

    } else if (command == "policyfeatflip") {
        g_policy.feat_swap = !g_policy.feat_swap;

        std::cout << "info string policy_feat_swap="
                  << (g_policy.feat_swap ? 1 : 0)
                  << std::endl;

    } else if (command == "policylayout") {
        if (tokens.size() >= 3) {
            try {
                g_policy.l1_hidden_major = (std::stoi(tokens[1]) != 0);
                g_policy.l2_out_major    = (std::stoi(tokens[2]) != 0);

                std::cout << "info string policylayout"
                          << " l1_hidden_major=" << (g_policy.l1_hidden_major ? 1 : 0)
                          << " l2_out_major="    << (g_policy.l2_out_major ? 1 : 0)
                          << std::endl;
            } catch (...) {
                std::cout << "info string invalid policylayout args" << std::endl;
            }
        } else {
            std::cout << "info string usage: policylayout <l1_hidden_major> <l2_out_major>"
                      << std::endl;
        }

    } else if (command == "policysee") {
        if (tokens.size() >= 2) {
            try {
                g_policy.see_invert = (std::stoi(tokens[1]) != 0);

                std::cout << "info string policy_see_invert="
                          << (g_policy.see_invert ? 1 : 0)
                          << std::endl;
            } catch (...) {
                std::cout << "info string invalid policysee arg" << std::endl;
            }
        } else {
            std::cout << "info string usage: policysee <0|1>" << std::endl;
        }

    } else if (command == "policyfeat") {
        if (tokens.size() >= 2) {
            try {
                g_policy.feat_swap = (std::stoi(tokens[1]) != 0);

                std::cout << "info string policy_feat_swap="
                          << (g_policy.feat_swap ? 1 : 0)
                          << std::endl;
            } catch (...) {
                std::cout << "info string invalid policyfeat arg" << std::endl;
            }
        } else {
            std::cout << "info string usage: policyfeat <0|1>" << std::endl;
        }

    } else if (command == "policyact") {
        if (tokens.size() >= 2) {
            try {
                g_policy.act_mode = std::stoi(tokens[1]);

                std::cout << "info string policy_act_mode="
                          << g_policy.act_mode
                          << " (0=crelu01, 1=relu, 2=identity, 3=screlu01)"
                          << std::endl;
            } catch (...) {
                std::cout << "info string invalid policyact arg" << std::endl;
            }
        } else {
            std::cout << "info string usage: policyact <0|1|2|3>" << std::endl;
        }
...
void uci_loop(int argc, char* argv[]) {
    ...
    if (!g_policy.loadFromMemory(g_policy_embed_data,
                                 g_policy_embed_size,
                                 "embedded")) {
