...
bool PolicyNet::loadFromMemory(const std::uint8_t* data, std::size_t size, const char* label) {
    ...
    const size_t expected =
        ...
        size_t(POLICY_HL) +
        size_t(POLICY_HL) * size_t(POLICY_HL_PAIR) +
        size_t(POLICY_HL) +
    ...
    if (!read_q(l0w, size_t(POLICY_INPUT_SIZE) * size_t(POLICY_HL), off) ||
        ...
        !read_q(l1w, size_t(POLICY_HL) * size_t(POLICY_HL_PAIR), off) ||
        !read_q(l1b, size_t(POLICY_HL), off) ||
        !read_q(l2w, size_t(POLICY_HL_PAIR) * size_t(num_moves), off) ||
    ...
    std::cout << "info string Policy loaded " << (label ? label : "memory")
              ...
              << " layers=3"
              << " bytes=" << size
              << " mode=root_lmr+tm+cached_killer"
...
// ============================================================================
// Near-root cached advice
// ============================================================================
bool PolicyNet::nearRootAdvice(const chess::Board& board, PolicyAdvice& out) const {
    out.reset();
    ...
    for (int i = 0; i < n; ++i) {
        const int mi = mapMoveToIndex(board, moves[i]);
        logits[i] = (mi >= 0) ? logitForMoveIndex(*this, h, mi) : -1e9f;

        if (logits[i] > best_any_logit) {
            best_any_logit = logits[i];
            best_any_i = i;
        }

        if (policyQuietLocal(board, moves[i])) {
            qidx[nq++] = i;
        }
    }

    if (best_any_i >= 0) {
        out.top_any = moves[best_any_i];
    }
    ...
    float max_q = -1e30f;
    for (int j = 0; j < nq; ++j) {
        max_q = std::max(max_q, logits[qidx[j]]);
    }
    ...
    for (int j = 0; j < nq; ++j) {
        probs[j] = std::exp(logits[qidx[j]] - max_q);
        sum += probs[j];
    }
    ...
    for (int r = 0; r < nq; ++r) {
        const int q = order[r];
        const float p = probs[q] / sum;

        if (p > 1e-12f) {
            ent -= double(p) * std::log(double(p));
        }

        if (r < POLICY_KILLER_MAX_MOVES) {
            out.quiets[r] = moves[qidx[q]];
            out.probs[r] = p;
        }
    }

    out.count = std::min(POLICY_KILLER_MAX_MOVES, nq);
    out.top_prob = out.probs[0];

    float norm_ent = 0.0f;
    if (nq > 1) {
        norm_ent = static_cast<float>(ent / std::log(static_cast<float>(nq)));
    }

    float sharpness = std::clamp((0.90f - norm_ent) / 0.35f, 0.0f, 1.0f);
    if (out.top_prob < 0.12f) sharpness *= 0.5f;
...
void PolicyNet::debugPosition(const chess::Board& board, int topN) const {
...

    {
        ...
        for (int i = 0; i < 8 && i < POLICY_HL_PAIR; ++i) std::cout << " " << h[i];
        std::cout << std::endl;
    }
