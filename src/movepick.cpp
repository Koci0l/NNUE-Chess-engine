...
MovePicker::MovePicker(const chess::Board& board, const MovePickerContext& ctx,
                       int depth, bool skip_quiets, bool)
    : m_board(board), m_ctx(ctx), m_depth(depth), m_skip_quiets(skip_quiets),
      ...
      m_last_score(0), m_last_policy_rel(0.0f), m_returned_count(0),
...
void MovePicker::scoreQuiets() {
    ...
    for (const auto& move : m_all_legal) {
        if (move == m_ctx.tt_move) continue;
        if (move == m_ctx.policy_hint) continue;
...
chess::Move MovePicker::next(bool& is_quiet_out) {
    ...
    while (true) {
        ...
        case MovePickStage::COUNTER_MOVE: {
            m_stage = MovePickStage::POLICY_HINT;
        ...
        case MovePickStage::POLICY_HINT: {
            m_stage = MovePickStage::GENERATE_QUIETS;
            if (!m_skip_quiets && m_ctx.policy_hint != chess::Move() &&
                m_ctx.policy_hint != m_ctx.tt_move &&
                m_ctx.policy_hint != m_killer1 &&
                m_ctx.policy_hint != m_killer2 &&
                m_ctx.policy_hint != m_ctx.counter_move) {
                ensureLegal();
                if (isValid(m_ctx.policy_hint)) {
                    bool is_capture = m_board.at(m_ctx.policy_hint.to()) != chess::Piece::NONE ||
                                      m_ctx.policy_hint.typeOf() == chess::Move::ENPASSANT ||
                                      m_ctx.policy_hint.typeOf() == chess::Move::PROMOTION;
                    if (!is_capture) {
                        m_last_score = 1200000;
                        m_last_policy_rel = m_ctx.policy_hint_rel;
                        is_quiet_out = true;
                        return m_ctx.policy_hint;
                    }
                }
            }
            break;
