...
// ============================================================================
// Policy bonus for in-tree quiet ordering
// ============================================================================
static inline int policyBonus(const RootPolicy& np, const chess::Move& move) {
    if (!np.ok) return 0;

    const int idx = np.find(move);
    if (idx < 0) return 0;

    const int rank = np.quiet_rank[idx];
    if (rank < 0) return 0;  // not a quiet

    // rel = log(p_quiet * nq): 0 means "average quiet", positive = better
    const float rel = np.rel[idx];
    const float sharp = np.quiet_sharpness;

    // Scale by sharpness so we don't trust the net when distribution is flat
    int bonus = int(1200.0f * rel * sharp);

    // Rank bonus: extra kick for the very top moves
    int rank_bonus = 0;
    if (rank == 0)      rank_bonus = 5000;
    else if (rank == 1) rank_bonus = 3000;
    else if (rank == 2) rank_bonus = 1500;
    else if (rank <= 5) rank_bonus = 600;
    else if (rank <= 10) rank_bonus = 150;

    bonus += int(float(rank_bonus) * sharp);

    return std::clamp(bonus, -6000, 10000);
}
...
void MovePicker::scoreQuiets() {
    ...
    // ========================================================================
    // In-tree policy: compute once per node, only when depth justifies it.
    // At depth >= 6, the subtree is large enough that one policy eval
    // (~10-20 µs scalar) is amortized over thousands of descendant nodes.
    // ========================================================================
    const bool use_policy = g_policy.loaded &&
                            m_depth >= POLICY_TREE_MIN_DEPTH &&
                            !m_skip_quiets;

    if (use_policy) {
        computeRootPolicy(m_board, m_np);
        // computeRootPolicy sets m_np.ok = false if anything fails
    } else {
        m_np.ok = false;
    }
    ...
    for (const auto& move : m_all_legal) {
        ...
        int s = scoreOneQuiet(move);

        // Add policy bonus on top of history
        if (m_np.ok) {
            s += policyBonus(m_np, move);
        }

        m_quiets[m_quiet_count].move = move;
        m_quiets[m_quiet_count].score = s;
        ++m_quiet_count;
    }
}
