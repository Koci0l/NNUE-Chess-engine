...
MovePicker::MovePicker(const chess::Board& board, const MovePickerContext& ctx,
                       int depth, bool skip_quiets, bool use_small_policy)
    : m_board(board), m_ctx(ctx), m_depth(depth), m_skip_quiets(skip_quiets),
      ...
      m_use_small_policy(use_small_policy),
      m_small_attempted(false) {
}
...
int MovePicker::scoreOneQuiet(const chess::Move& move, int legal_idx) {
    (void)move;
    ...
    // Mild only: slightly reorder normal quiets.
    // ------------------------------------------------------------------
    if (m_small.ok &&
        m_small.sharp >= POLICY_SMALL_SEARCH_SHARP_MIN &&
        legal_idx >= 0 &&
        legal_idx < 256) {
        const int r = m_small.rank[legal_idx];

        if (r >= 0) {
            const float rel = m_small.rel[legal_idx];
            const float sharp = m_small.sharp;

            int bonus = static_cast<int>(200.0f * rel * sharp);

            int rank_bonus = 0;
            if (r == 0)       rank_bonus = 400;
            else if (r == 1)  rank_bonus = 250;
            else if (r == 2)  rank_bonus = 150;
            else if (r <= 5)  rank_bonus = 60;
            else if (r <= 10) rank_bonus = 15;

            bonus += static_cast<int>(rank_bonus * sharp);

            score += std::clamp(bonus, -600, 900);
        }
    }
...
void MovePicker::scoreQuiets() {
    ...
    for (int legal_idx = 0; legal_idx < static_cast<int>(m_all_legal.size()); ++legal_idx) {
        const auto& move = m_all_legal[legal_idx];
        ...
        m_quiets[m_quiet_count].score = scoreOneQuiet(move, legal_idx);
        ++m_quiet_count;
    }
}

void MovePicker::ensureSmallPolicy() {
    if (!m_use_small_policy || m_small_attempted) return;

    m_small_attempted = true;

    if (m_skip_quiets) return;
    if (m_depth < POLICY_SMALL_SEARCH_MIN_DEPTH) return;
    if (m_depth > POLICY_SMALL_SEARCH_MAX_DEPTH) return;
    if (m_board.inCheck()) return;

    ensureLegal();

    if (m_all_legal.size() == 0 || m_all_legal.size() > 256) return;

    computeSmallPolicyView(m_board, m_all_legal, m_small);
}

int MovePicker::legalIndex(const chess::Move& move) const {
    if (!m_legal_generated) return -1;

    for (int i = 0; i < static_cast<int>(m_all_legal.size()); ++i) {
        if (m_all_legal[i] == move) return i;
    }
...
int scoreMoveForOrdering(const chess::Board& board, const chess::Move& move,
    ...
    if (move == ctx.tt_move && ctx.tt_move != chess::Move()) return 1000000;
    ...
    if (killer_score > 0) return 700000 + killer_score * 1000;

    if (move == ctx.counter_move && ctx.counter_move != chess::Move()) return 650000;

    int score = g_butterflyHistory.get(ctx.side_to_move, move.from(), move.to());

    if (move.typeOf() == chess::Move::CASTLING) score += 50;
...
void pickNextMove(std::vector<ScoredMove>& moves, size_t current) {
    ...
    if (best_idx != current) std::swap(moves[current], moves[best_idx]);
}