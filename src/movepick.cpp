...
int MovePicker::scoreOneQuiet(const chess::Move& move) {
    ...
    // Policy ordering bonus
    if (m_ctx.policy && m_ctx.policy->ok) {
        const PolicyAdvice& pa = *m_ctx.policy;

        int r = pa.rank_of(move);

        if (r < 999) {
            float rel = pa.quiet_rel[r];
            float sharp = pa.sharpness;

            int policy_bonus = int(POLICY_ORDER_WEIGHT * rel * sharp);
            policy_bonus = std::clamp(policy_bonus, -4000, 6000);
            ...
            // Move not in policy top-K: mild penalty if policy is sharp
            if (pa.sharpness > 0.4f) {
                score -= int(1500.0f * pa.sharpness);
            }
        }
    }
