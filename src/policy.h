...
// ============================================================================
// Cached policy killer / protected move near root
// ============================================================================
constexpr int   POLICY_KILLER_MIN_DEPTH  = 8;
constexpr int   POLICY_KILLER_MAX_PLY    = 2;
constexpr float POLICY_KILLER_MIN_SHARP  = 0.15f;
constexpr float POLICY_PROTECT_MIN_SHARP = 0.35f;
constexpr int   POLICY_KILLER_MAX_MOVES  = 3;

// ============================================================================
// PolicyAdvice: cached near-root policy suggestion
// ============================================================================
struct PolicyAdvice {
    bool ok = false;
    int count = 0;

    chess::Move quiets[POLICY_KILLER_MAX_MOVES];
    float probs[POLICY_KILLER_MAX_MOVES];
    ...
    void reset() {
        ...
        for (int i = 0; i < POLICY_KILLER_MAX_MOVES; ++i) {
            quiets[i] = chess::Move();
            probs[i] = 0.0f;
        }
    ...
    int rankOf(const chess::Move& m) const {
        if (m == chess::Move()) return -1;

        for (int i = 0; i < count; ++i) {
            if (quiets[i] == m) return i;
        }

        return -1;
    }

    bool protectedMove(const chess::Move& m) const {
        if (!ok) return false;
        if (sharpness < POLICY_PROTECT_MIN_SHARP) return false;
        return rankOf(m) == 0;
    }
};
...
struct PolicyNet {
    ...
    bool nearRootAdvice(const chess::Board& board, PolicyAdvice& out) const;
