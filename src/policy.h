...
constexpr int   POLICY_IIR_MIN_DEPTH  = 8;    // no cache yet -> keep gate tight
constexpr int   POLICY_IIR_MAX_PLY    = 3;    // near-root only
constexpr float POLICY_IIR_MIN_MARGIN = 0.0f; // min (top1-top2) logit gap; 0.0 = disabled
...
struct PolicyNet {
    ...
    // [C] Cheap top-1 over all legal moves + confidence margin (no softmax, no entropy).
    bool topMoveMargin(const chess::Board& board,
                       chess::Move& out_move,
